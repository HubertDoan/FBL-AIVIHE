...
/**
 * Reception/admin/director update consultation request
 * - Reception: status 'new' → 'contacted' → 'info_completed' (điền extended_info)
 * - Director: status 'info_completed' → 'approved' or 'rejected'
 *
 * PATCH /api/consultation-request/:id
 * Body: { status?, extended_info?, rejected_reason?, ... }
 */
...
function getDemoUserFromCookie(request: NextRequest) {
  const cookie = request.cookies.get('demo-auth-session')
  if (!cookie?.value) return null
  try {
    return JSON.parse(cookie.value) as { id: string; role: string }
  } catch {
    return null
  }
}
...
export async function GET(
  ...
  if (isDemoMode()) {
    const req = getConsultationRequestById(id)
...
export async function PATCH(
  ...
  if (isDemoMode()) {
    ...
    const existing = getConsultationRequestById(id)
    ...
    if (Object.keys(patch).length === 0) {
      return NextResponse.json(
        { error: 'Không có thay đổi hợp lệ cho role này' },
        { status: 400 }
      )
    }

    const updated = updateConsultationRequest(id, patch)
  ...
  // Similar patch logic (would insert into Supabase)
